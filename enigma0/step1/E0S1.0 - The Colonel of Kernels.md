...
If an activation is triggered _asynchronously_ it can either be executed in one of several ways.  The first is an
...
As before, each activation just returns back the beat number from which it started. Since it was instantiated
on impulse beat 42, _that_ value gets associated with the starting grid position on the x-axis regardless of
...
It truly is that simple, but the concept is _powerful_ - I'll circle back to this later -
