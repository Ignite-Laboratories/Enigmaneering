...
the resulting delta has?  One could argue that this only provides an average of '1' _because_ the sign bit 
transiently appears!  If only we could consider the sign as an entirely separate _artifact_ and store the
**_absolute value_** of the delta, then we'd _technically_ be achieving an average bit drop of _'2'..._

The next solution achieves that.