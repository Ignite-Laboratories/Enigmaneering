...
it.  The _next size up_ date range  (64 bits) covers 584 _billion_ years in either direction from epoch!
...
These are limited to the host architecture's bit-width, allowing them to be considered a _single_
computable value.

**Phrases** - A phrase is a _slice_ of measurements, allowing arbitrary length storage of binary information.

**Movements** - A movement is a _logical transformation_ of binary information, but we'll get to that shortly.
...
For ease, these are the names of the sub-byte sizes which I've adopted:
